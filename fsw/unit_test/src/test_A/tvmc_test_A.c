...
void TVMC_Pack_0x19BA_Test(void)
{
    const char *outputs[3] = {
        "trick.var_set('dyn.cannon.vel[0]', 0.123400) \n",
        "trick.var_set('dyn.cannon.vel[1]', 5.432100) \n",
        "trick.var_set('trick_zero_conf.zc.name', 'hello bob o/') \n"
    };
    
    Struct_Cannon_SetVelCmd cmd;

    cmd.vel[0] = 0.123400;
    cmd.vel[1] = 5.432100;

    sprintf(cmd.mystring, "hello bob o/");
    ...
    for (int i = 0; i < 3; ++i)
    {
        UtAssert_StrCmp(packedCommandBuffer[i], outputs[i], "strcmp test A");
    }
